...
def train(model, model_par, criterion, optimizer):
    ...
    for epoch in range(config.epoch_num):
        # 数据集构建
        built_dataset(config.txt_folder, config.train_data_path, config.dev_data_path, config.max_len)
        train_dataset = MTDataset(config.train_data_path, config.mask_prob)
        dev_dataset = MTDataset(config.dev_data_path, config.mask_prob)
...
def translate(src, model, use_beam=True):
    ...
    _, tgt_decode = target_tokenizer_load()
    with torch.no_grad():
        model.load_state_dict(torch.load(lastest_checkpoint(), map_location=config.device))
        model.eval()
        src_mask = (src != 0).unsqueeze(-2)
        if use_beam:
            decode_result, _ = beam_search(model, src, src_mask, config.max_len,
